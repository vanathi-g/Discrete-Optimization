...
indices = ['x{0}{1}'.format(row,col) for row in range(9) for col in range(9)]
...
for c in range(9):
	...
	rowIndices = [globals()['x{0}{1}'.format(num,col)] for col in range(9)]
	model.AddAllDifferent(rowIndices)

	colIndices = [globals()['x{0}{1}'.format(row,num)] for row in range(9)]
...
while(rowNum < 9):
	colNum = 0
	while(colNum < 9):
		boxIndices = [globals()['x{0}{1}'.format(row,col)] for row in range(rowNum, rowNum+3) for col in range(colNum, colNum+3)]
